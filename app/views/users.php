<?php
    var_dump($_SESSION);
    echo '<br>';
    var_dump($_SESSION['auth_user_id']);
    echo '<br>';
    var_dump($_SESSION['auth_user_id']);
...
<main id="js-page-content" role="main" class="page-content mt-3">
    ...
    <div class="row">
        <div class="col-xl-12">
            ...
            <?php if ($_SESSION['auth_logged_in'] == true && ($_SESSION['auth_roles'] == 1)): ?>
    ...
    <div class="row" id="js-contacts">
        ...
        <?php foreach ($users as $user): ?> <!-- $postsInView - в виде обращаюсь к переменной по названию ключа -->
            <div class="col-xl-4">
                <div id="c_1" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="<?php echo $user['data_filter_tag']; ?>">
                    <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                        <div class="d-flex flex-row align-items-center">
                            <!-- status_active - c БД, тут используем его -->
                                <span class="status status-<?php echo $user['status_active']; ?> mr-3">
                            ...
                            <div class="info-card-text flex-1">
                                ...
                                <div class="dropdown-menu">
                                    <input type="text" value="<?=  $user['id'];?>" /> <!-- временно вывели -->
                                    ...
                                    <a class="dropdown-item" href="/status?id=<?= $user['id']; ?>">
                                        <i class="fa fa-sun"></i>
                                        Установить статус</a>
                                    <a class="dropdown-item" href="/media_page?id=<?= $user['id']; ?>">
                                ...
                                <span class="text-truncate text-truncate-xl"><?php echo $user['job_title']; ?>, <?php echo $user['company']; ?></span>
                            </div>
                            <?php endif; ?>

                            <div class="info-card-text flex-1">
                                <?php echo $user['username']; ?>
                                <?php if ($_SESSION['auth_user_id'] == $user['id'] &&($_SESSION["auth_roles"] != 1)): ?>
                                <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info" data-toggle="dropdown" aria-expanded="false">
                                    <i class="fal fas fa-cog fa-fw d-inline-block ml-1 fs-md"></i>
                                    <i class="fal fa-angle-down d-inline-block ml-1 fs-md"></i>
                                </a>
                                ...
                                <div class="dropdown-menu">
                                    <input type="text" value="<?=  $user['id'];?>" /> <!-- временно вывели -->
                                    <a class="dropdown-item" href="/edit?id=<?= $_SESSION['auth_user_id']; ?>">
                                    <input type="hidden" <?php $_SESSION['user_id'] = $user['id'] ?> />
                                        <i class="fa fa-edit"></i>
                                        Редактировать</a>
                                    <a class="dropdown-item" href="/security?id=<?= $user['id']; ?>">
                                        <i class="fa fa-lock"></i>
